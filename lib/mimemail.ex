defmodule MimeMail do
  # ever the raw line or any term implementing MimeMail.Header.to_ascii
  @type header :: {:raw, binary} | MimeMail.Header.t()
  # ever the raw body or list of mail for multipart or binary for decoded content
  @type body :: binary | [MimeMail.t()] | {:raw, binary}
  ...
  def encode_body(%MimeMail{body: body} = mail) when is_binary(body) do
    ...
    case mail.headers[:"content-type"] do
      {"text/" <> _ = type, params} ->
        headers =
          Keyword.drop(mail.headers, [:"content-type", :"content-transfer-encoding"]) ++
            [
      ...
      _ ->
        headers =
          Keyword.delete(mail.headers, :"content-transfer-encoding") ++
            ["content-transfer-encoding": "base64"]

        %{
          mail
          | headers: headers,
            body: {:raw, body |> Base.encode64() |> chunk64 |> Enum.join("\r\n")}
        }
    end
  end
  ...
  def encode_body(%MimeMail{body: childs} = mail) when is_list(childs) do
    ...
    headers =
      Keyword.delete(mail.headers, :"content-type") ++
        ["content-type": {type, Map.put(params, :boundary, boundary)}]

    body =
      childs
      |> Enum.map(&MimeMail.to_string/1)
      |> Enum.join("\r\n\r\n" <> full_boundary <> "\r\n")

    %{
      mail
      | body: {:raw, "#{full_boundary}\r\n#{body}\r\n\r\n#{full_boundary}--\r\n"},
        headers: headers
    }
  end
  ...
  defp chunk_line(<<vline::size(74)-binary, ?=, rest::binary>>),
    do: vline <> "=\r\n" <> chunk_line("=" <> rest)

  defp chunk_line(<<vline::size(75)-binary, rest::binary>>),
    do: vline <> "=\r\n" <> chunk_line(rest)

  defp chunk_line(other), do: other

  def qp_to_binary(str),
    do: str |> String.trim_trailing() |> String.trim_trailing(<<?=::utf8>>) |> qp_to_binary([])

  def qp_to_binary("=\r\n" <> rest, acc), do: qp_to_binary(rest, acc)

  def qp_to_binary(<<?=, x1, x2>> <> rest, acc),
    do: qp_to_binary(rest, [<<x1, x2>> |> String.upcase() |> Base.decode16!() | acc])

  def qp_to_binary(<<c, rest::binary>>, acc), do: qp_to_binary(rest, [c | acc])
  def qp_to_binary("", acc), do: acc |> Enum.reverse() |> IO.iodata_to_binary()

  def unfold_header(value), do: String.replace(value, ~r/\r\n([\t ])/, "\\1")

  def fold_header(header),
  ...
  def fold_header(other, acc),
    do: (acc |> List.flatten() |> Enum.reverse() |> Kernel.to_string()) <> other
  ...
  def encode_header_key(key),
    do: String.split("#{key}", "-") |> Enum.map(&header_key/1) |> Enum.join("-")

  # acronym, upcase
  def header_key(word) when word in ["dkim", "spf", "x", "id", "mime"], do: String.upcase(word)
  # not acronym, camelcase
  def header_key(word), do: String.capitalize(word)
end
...
defmodule Iconv do
  ...
  def init do
    ...
    case ret do
      {:error, {:load_failed, _}} ->
        if Code.ensure_loaded?(Codepagex),
          do: :ok,
          else: {:error, "Codepagex is not available. Cannot fallback."}
  ...
  @spec conv(binary, binary, binary) ::
          binary | {:error, term} | {:error, binary, term} | {:incomplete, binary, binary}
